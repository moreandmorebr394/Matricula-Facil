...
class SistemaCurso:
    ...
    def __init__(self, raiz):
        self.raiz = raiz
        self.raiz.title("Sistema Fácil — Gestão de Cursos")
    ...
    def _configurar_estilos(self):
        ...
        estilo.configure(
            "Moderno.TCombobox",
            fieldbackground=self.COR_FUNDO,
        ...
        estilo.map(
            "Moderno.TCombobox",
            fieldbackground=[("readonly", self.COR_FUNDO)],
            selectbackground=[("readonly", self.COR_FUNDO)],
    ...
    def _construir_sidebar(self, parent):
        ...
        itens = [
            ...
            ("\U0001F50D  Consultar Curso", self.funcao_quadro_busca),
    ...
    def _construir_topbar(self, parent):
        ...
        tk.Label(
            esquerda,
            text="Cadastre, consulte, edite e remova cursos da instituição",
            bg=self.COR_FUNDO,
    ...
    def _construir_cards_resumo(self, parent):
        ...
        self.card_total = self._criar_card_metric(
            wrap, "Total de Cursos", "—", self.COR_ROYAL_BLUE
        )
        self.card_total.pack(side="left", expand=True, fill="x", padx=(0, 12))

        self.card_tecnicos = self._criar_card_metric(
            wrap, "Cursos Técnicos", "—", self.COR_QUICKSAND
        )
        self.card_tecnicos.pack(side="left", expand=True, fill="x", padx=6)

        self.card_livres = self._criar_card_metric(
            wrap, "Cursos Livres", "—", self.COR_SUCESSO
        )
        self.card_livres.pack(side="left", expand=True, fill="x", padx=(12, 0))
    ...
    def _construir_painel_tabela(self, parent):
        ...
        cabecalhos = ["ID", "Nome", "Duração", "Carga Horária", "Tipo"]
        larguras = [60, 260, 140, 160, 160]
    ...
    def funcao_quadro_adicionar(self):
        janela = self._criar_modal("Cadastrar Novo Curso", 520, 580)
        self.janela_form = janela

        self._titulo_modal(
            janela,
            "Novo Curso",
            "Preencha os dados abaixo para registrar um novo curso.",
        )
        ...
        self._campo_form(form, "Nome do curso", "nome", self.entradas)
        self._campo_form(form, "Duração", "duracao", self.entradas)
        self._campo_form(form, "Carga Horária", "carga_horario", self.entradas)
        self._campo_combobox(
            form,
            "Tipo de Curso",
            "tipo_curso",
            ["capacitacao", "livre", "tecnico"],
            self.entradas,
        )
    ...
    def _campo_combobox(self, parent, label, chave, valores, store):
        wrap = tk.Frame(parent, bg=self.COR_BRANCO)
        wrap.pack(fill="x", pady=(6, 10))
    ...
    def salvar_dados(self):
        try:
            ...
            valores = (
                ...
                self.entradas["carga_horario"].get(),
                self.entradas["tipo_curso"].get(),
            )
    ...
    def funcao_quadro_busca(self):
        janela = self._criar_modal("Consultar Curso", 460, 320)
        self.janela_busca = janela

        self._titulo_modal(
            janela,
            "Consultar Curso",
            "Digite parte do nome do curso para localizar registros.",
        )
        ...
        store = {}
        self._campo_form(form, "Nome do curso", "nome", store)
        self.ent_busca = store["nome"]
    ...
    def mostrar_todos(self):
        try:
            ...
            self._atualizar_cards(self._dados_atuais)
            self.lbl_status.configure(
                text=f"{len(self._dados_atuais)} curso(s) carregado(s)."
            )
        except Exception as e:
            self.lbl_status.configure(text=f"Erro ao carregar: {e}")
    ...
    def _atualizar_cards(self, linhas):
        total = len(linhas)
        # Colunas: id_curso, nome, duracao, carga_horario, tipo_curso
        tecnicos = sum(
            1 for l in linhas if len(l) > 4 and str(l[4]).lower() == "tecnico"
        )
        livres = sum(
            1 for l in linhas if len(l) > 4 and str(l[4]).lower() == "livre"
        )
        self.card_total.label_valor.configure(text=str(total))
        self.card_tecnicos.label_valor.configure(text=str(tecnicos))
        self.card_livres.label_valor.configure(text=str(livres))

    # ----------------------- UPDATE -----------------------
    def funcao_quadro_atualizar(self):
        janela = self._criar_modal("Editar Curso", 520, 600)
        self.janela_atua = janela

        self._titulo_modal(
            janela,
            "Editar Registro",
            "Informe o ID do curso e os novos dados que deseja atualizar.",
        )
        ...
        store = {}
        self._campo_form(form, "ID do Curso", "id", store)
        self._campo_form(form, "Novo Nome", "nome", store)
        self._campo_form(form, "Nova Duração", "duracao", store)
        self._campo_form(form, "Nova Carga Horária", "carga", store)

        self.id_editar = store["id"]
        self.novo_nome = store["nome"]
        self.nova_duracao = store["duracao"]
        self.nova_carga = store["carga"]
    ...
    def funcao_quadro_remover(self):
        janela = self._criar_modal("Excluir Curso", 420, 320)
