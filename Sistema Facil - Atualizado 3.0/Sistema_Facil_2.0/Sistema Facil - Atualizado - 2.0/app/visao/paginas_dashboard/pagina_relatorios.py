...
class PaginaRelatorios(tk.Frame):
    ...
    def _construir(self):
        ...
        self._canvas_main.create_window((0, 0), window=self._cont, anchor="nw",
                                        width=1140)
        self._cont.bind("<Configure>",
                        lambda e: self._canvas_main.configure(
                            scrollregion=self._canvas_main.bbox("all")))
    ...
    def _tabela_generica(self, parent, titulo, colunas, linhas, cor_titulo=AZUL_PRIMARIO):
        ...
        h = tk.Frame(card, bg=BRANCO, padx=18, pady=12)
        h.pack(fill="x")
        ...
        # Cabecalho
        head = tk.Frame(card, bg=cor_titulo)
