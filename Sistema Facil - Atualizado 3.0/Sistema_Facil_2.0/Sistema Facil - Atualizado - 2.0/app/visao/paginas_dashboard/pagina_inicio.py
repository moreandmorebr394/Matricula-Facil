...
class PaginaInicio(tk.Frame):
    ...
    def _desenhar_funil(self, card):
        """Desenha funil de vendas em Canvas - apenas com dados reais."""
        ...
        # Niveis do funil - apenas dados reais (sem valores hardcoded)
        niveis = [
            ("Visitantes", status_data.get("LEAD", 0),
             FUNIL_VISITANTES),
            ("Leads", status_data.get("LEAD", 0),
             FUNIL_LEADS),
            ("Negociacao", status_data.get("EM NEGOCIACAO", 0),
             FUNIL_NEGOCIACAO),
            ("Vendas", status_data.get("VENDA FECHADA", 0),
             FUNIL_VENDAS),
            ("Ativos", status_data.get("MATRICULADO", 0),
             FUNIL_ATIVOS),
        ]

        total_geral = sum(v for _, v, _ in niveis)

        # Se nao ha dados, exibe mensagem
        if total_geral == 0:
            canvas.create_text(
                240, 120,
        ...
        for i, (rotulo, valor, cor) in enumerate(niveis):
            ...
            largura = int(largura_total * (valor / max_val)) if max_val > 0 else 80
    ...
    def _desenhar_pizza(self, card):
        """Desenha grafico de pizza com origens de leads - apenas dados reais."""
        canvas = tk.Canvas(card, bg=BRANCO, height=240,
                           highlightthickness=0)
        canvas.pack(fill="both", expand=True, padx=18, pady=14)
