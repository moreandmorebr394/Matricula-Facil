...
class TelaDashboard(tk.Toplevel):
    ...
    def _construir_header(self):
        ...
        self.lbl_avatar_header = tk.Label(avatar, text="👨‍💼", font=("Segoe UI Emoji", 18),
                                          bg=AMARELO_VIBRANTE, cursor="hand2")
        self.lbl_avatar_header.pack(expand=True)
        ...
        for w in (perfil, avatar, info, self.lbl_avatar_header, self.lbl_nome_header):
            try:
                w.bind("<Button-1>",
    ...
    def atualizar_perfil_header(self):
        """Atualiza a foto e o nome do admin no header."""
        try:
        ...
        if hasattr(self, "lbl_avatar_header"):
            if self.foto_admin_b64:
                try:
                    ...
                    self.lbl_avatar_header.configure(image=photo, text="", bg=AMARELO_VIBRANTE)
                except Exception:
                    self.lbl_avatar_header.configure(image="", text="👨‍💼", font=("Segoe UI Emoji", 18), bg=AMARELO_VIBRANTE)
            else:
                self.lbl_avatar_header.configure(image="", text="👨‍💼", font=("Segoe UI Emoji", 18), bg=AMARELO_VIBRANTE)
