...
def obter_venda(venda_id):
    ...
    with obter_conexao() as conexao:
        cursor = obter_cursor(conexao, dicionario=True)
        cursor.execute(f"SELECT * FROM vendas WHERE id = {p}", (venda_id,))
...
def atualizar_venda(venda_id, dados):
    ...
    sql = f"UPDATE vendas SET {set_clausula} WHERE id = {p}"
    with obter_conexao() as conexao:
        cursor = obter_cursor(conexao, dicionario=False)
        cursor.execute(sql, valores)
        cursor.close()
...
def excluir_venda(venda_id):
    ...
    with obter_conexao() as conexao:
        cursor = obter_cursor(conexao, dicionario=False)
        cursor.execute(f"DELETE FROM vendas WHERE id = {p}", (venda_id,))
        cursor.close()
