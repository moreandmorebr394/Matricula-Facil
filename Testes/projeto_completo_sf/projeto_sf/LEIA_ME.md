...
Sistema completo de gestao educacional construido em **Python + Tkinter**, com banco de dados MySQL (e fallback automatico para SQLite). Inclui pagina inicial vitrine, login/cadastro de alunos, area do aluno e dashboard administrativo CRM completo.
...
- **Python 3.9+**
- **WampServer** (com MySQL ativo) — opcional; o sistema usa SQLite automaticamente se MySQL nao estiver disponivel
...
> ⚠️ **Sem WampServer/MySQL?** Sem problema! O sistema cria um arquivo SQLite local automaticamente em `banco_de_dados/sistema_facil.db`.
...
│   ├── conexao.py                  # MySQL + fallback SQLite
│   ├── inicializar.py              # Cria tabelas
│   ├── esquema.sql                 # SQL para Workbench
│   └── sistema_facil.db            # SQLite (criado em runtime)
│
...
- **Solucao 1:** Verifique se WampServer esta rodando (icone verde)
- **Solucao 2:** Edite `banco_de_dados/conexao.py` com suas credenciais
- **Solucao 3:** O sistema usa SQLite automaticamente — basta ignorar o aviso

### bcrypt nao instala (Windows)
```bash
pip install --upgrade pip setuptools wheel
pip install bcrypt
```
Se mesmo assim falhar, o sistema usa SHA-256 como fallback automatico.

### Nao consigo logar como admin
- Email: `admin@sistemafacil.pa.br`
- Senha: `admin123`
- Os campos sao **case-insensitive** para email
- Se voce alterou a senha pelo dashboard, ela fica no banco — apague o banco para resetar
