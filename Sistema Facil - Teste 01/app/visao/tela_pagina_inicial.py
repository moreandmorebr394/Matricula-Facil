...
class TelaPaginaInicial(tk.Tk):
    ...
    def _construir_conteudo(self):
        """Conteudo scrollavel."""
        canvas_principal = tk.Canvas(self, bg=BRANCO,
        ...
        # Frame interno
        self.conteudo = tk.Frame(canvas_principal, bg=BRANCO)
        canvas_principal.create_window((0, 0), window=self.conteudo,
        ...
        def ao_rolar(evento):
            canvas_principal.yview_scroll(int(-1 * (evento.delta / 120)),
                                          "units")
        canvas_principal.bind_all("<MouseWheel>", ao_rolar)
    ...
    def _secao_banner(self):
        ...
        for i in range(460):
            ...
            banner.create_line(0, i, 1200, i, fill=cor)
