...
class TelaLoginAdmin(tk.Toplevel):
    ...
    def _construir(self):
        # Header escuro
        header = tk.Canvas(self, width=520, height=180, bg=AZUL_ESCURO,
        ...
        for i in range(180):
            ...
            header.create_line(0, i, 520, i, fill=cor)

        # Decoracoes
        header.create_oval(-50, -50, 100, 100,
                           fill="#2D3F66", outline="")
        header.create_oval(420, 100, 600, 280,
                           fill="#2D3F66", outline="")

        # Linhas decorativas
        for x in range(0, 520, 22):
            header.create_oval(x, 160, x + 3, 163,
                               fill=AMARELO_VIBRANTE, outline="")

        # Logo
        logo_frame = tk.Frame(header, bg=AZUL_ESCURO)
        header.create_window(260, 60, window=logo_frame)
        LogoSF(logo_frame, tamanho=70, cor_fundo=AZUL_ESCURO).pack()

        # Cadeado e titulo
        header.create_text(260, 120, text="🔐",
                           font=("Segoe UI Emoji", 22))
        header.create_text(260, 150, text="Acesso Administrativo",
                           fill=BRANCO, font=(FONTE_TITULO, 16, "bold"))
