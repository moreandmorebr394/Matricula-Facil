...
def restaurar_registro(tabela, dados):
    """
    Restaura um registro genérico em qualquer tabela usando o dicionário de dados.
    """
    if not dados:
    ...
    try:
        colunas_validas = {
            "leads": [
                "id", "nome_completo", "data_nascimento", "cpf", "email", "telefone",
                "endereco", "numero", "complemento", "cidade", "bairro", "cep",
                "estado", "curso_interesse", "como_conheceu", "captador", "observacoes",
                "status", "data_cadastro"
            ],
            "vendas": [
                "id", "lead_id", "nome_aluno", "curso", "valor_total",
                "forma_pagamento", "parcelas", "status_pagamento",
                "vendedor", "observacoes", "data_venda"
            ],
            "pagamentos": [
                "id", "venda_id", "nome_aluno", "valor", "data_pagamento",
                "forma_pagamento", "parcela_numero", "status", "observacoes", "data_registro"
            ],
            "turmas": [
                "id", "nome_turma", "curso", "professor", "horario", "sala",
                "data_inicio", "data_fim", "capacidade_maxima",
                "alunos_matriculados", "status"
            ],
            "aulas": [
                "id", "turma_id", "titulo", "descricao", "data_aula",
                "horario_inicio", "horario_fim", "professor", "sala", "status"
            ]
        }.get(tabela)

        if not colunas_validas:
            return False, f"Tabela '{tabela}' desconhecida"

        colunas = [c for c in colunas_validas if c in dados]
        valores = tuple(dados[c] for c in colunas)
