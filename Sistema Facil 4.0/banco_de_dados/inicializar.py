...
def _garantir_colunas_leads(cursor):
    """
    Garante que as colunas numero, complemento, bairro e cep existam
    na tabela leads, mesmo em bancos criados antes desta atualizacao.
    Nao remove nem altera nenhuma coluna existente.
    """
    colunas_novas = {
        "numero": "ALTER TABLE leads ADD COLUMN numero VARCHAR(20) AFTER endereco",
        "complemento": "ALTER TABLE leads ADD COLUMN complemento VARCHAR(100) AFTER numero",
        "bairro": "ALTER TABLE leads ADD COLUMN bairro VARCHAR(100) AFTER cidade",
        "cep": "ALTER TABLE leads ADD COLUMN cep VARCHAR(15) AFTER bairro",
    }
    try:
        cursor.execute("SHOW COLUMNS FROM leads")
        existentes = {linha[0] for linha in cursor.fetchall()}
    except Exception as e:
        print(f"[Aviso] Nao foi possivel verificar colunas de leads: {e}")
        return

    for coluna, sql_alter in colunas_novas.items():
        if coluna not in existentes:
            try:
                cursor.execute(sql_alter)
                print(f"[OK] Coluna '{coluna}' adicionada na tabela leads.")
            except Exception as e:
                print(f"[Aviso] Falha ao adicionar coluna {coluna}: {e}")
...
def inicializar_banco():
    ...
    with obter_conexao() as conexao:
        ...
        # Garante colunas novas de endereco em bancos ja existentes
        _garantir_colunas_leads(cursor)
