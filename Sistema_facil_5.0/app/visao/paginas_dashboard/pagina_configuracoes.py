...
class PaginaConfiguracoes(tk.Frame):
    ...
    def _construir(self):
        ...
        def ar(_=None):
            canvas_main.update_idletasks()
            h_req = cont.winfo_reqheight()
            canvas_main.configure(scrollregion=(0, 0, 0, h_req))
    ...
    def _construir_perfil(self, parent):
        ...
        # Avatar com botao de upload (Canvas circular)
        avatar_wrap = tk.Frame(cont, bg=BRANCO)
        avatar_wrap.pack(pady=(0, 14))

        avatar_canvas = tk.Canvas(avatar_wrap, bg=BRANCO, width=110, height=110,
                                  highlightthickness=0, bd=0)
        avatar_canvas.pack()
        self._avatar_canvas = avatar_canvas
        ...
        if foto_b64 and _PIL_OK:
            ...
            self._desenhar_fallback_avatar()
    ...
    def _aplicar_foto_label(self, b64: str):
        """Decodifica o base64 e atualiza o Canvas do avatar com a imagem circular."""
        if not _PIL_OK or not hasattr(self, "_avatar_canvas"):
            return
        try:
            ...
            img = ImageOps.fit(img, (104, 104), Image.Resampling.LANCZOS)
            
            # Máscara circular
            mask = Image.new("L", (104, 104), 0)
            draw = ImageDraw.Draw(mask)
            draw.ellipse((0, 0, 104, 104), fill=255)
            img.putalpha(mask)
            ...
            # Borda circular
            self._avatar_canvas.create_oval(3, 3, 107, 107, outline=AZUL_PRIMARIO, width=3)
            # Imagem circular
            self._avatar_canvas.create_image(55, 55, image=photo, anchor="center")
        except Exception:
            self._desenhar_fallback_avatar()

    def _desenhar_fallback_avatar(self):
        if not hasattr(self, "_avatar_canvas"):
            return
        self._avatar_canvas.delete("all")
        # Borda circular
        self._avatar_canvas.create_oval(3, 3, 107, 107, outline=AZUL_PRIMARIO, width=3)
        # Círculo de fundo amarelo
        self._avatar_canvas.create_oval(6, 6, 104, 104, fill=AMARELO_VIBRANTE, outline="")
        # Emoji centralizado
        self._avatar_canvas.create_text(55, 55, text="👨‍💼", font=("Segoe UI Emoji", 50), fill=PRETO_TEXTO)
    ...
    def _trocar_foto(self):
        ...
        try:
            ...
            if _PIL_OK and hasattr(self, "_avatar_canvas"):
